...
test('uv, armv and libc tags from env var', function (t) {
  ...
  process.env.PREBUILD_TAG_ARMV = 'true' // Should be excluded (unless you run these tests on ARM)
  process.env.PREBUILD_TAG_LIBC = 'true' // Should be glibc (unless you run these tests on Alpine)
  prebuildify({
    cwd: path.join(__dirname, 'package'),
    targets: [{ runtime: 'node', target: process.version }]
  }, function (err) {
